...
static uint_fast8_t synthPeriod;
static uint_fast16_t synthEnergy;

#ifdef FAST_8BIT_MODE
static int8_t synthK1, synthK2;
#else
static int_fast16_t synthK1, synthK2;
#endif
static int_fast8_t synthK3, synthK4, synthK5, synthK6, synthK7, synthK8, synthK9, synthK10;
...
#if ISR_RATIO < 255
static uint_fast8_t ISRCounterToNextData = 0;
#else
...
#define ANALOG_WRITE_DESTINATION DAC0
...
#endif
    static uint_fast8_t periodCounter;
    static int_fast16_t x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;

    int_fast16_t u0, u1, u2, u3, u4, u5, u6, u7, u8, u9, u10;
...
#error One of PWM_VALUE_DESTINATION or ANALOG_WRITE_DESTINATION must be defined in initializeHardware()
    ...
    if (synthPeriod) {
        ...
        static uint_fast16_t synthRand = 1;
...
// rescale by shifting >> 7 because we have signed values here (for unsigned it would need >> 8)
    u9 = u10 - (((int_fast16_t) synthK10 * x9) >> 7);
    u8 = u9 - (((int_fast16_t) synthK9 * x8) >> 7);
    u7 = u8 - (((int_fast16_t) synthK8 * x7) >> 7);
    u6 = u7 - (((int_fast16_t) synthK7 * x6) >> 7);
    u5 = u6 - (((int_fast16_t) synthK6 * x5) >> 7);
    u4 = u5 - (((int_fast16_t) synthK5 * x4) >> 7);
    u3 = u4 - (((int_fast16_t) synthK4 * x3) >> 7);
    u2 = u3 - (((int_fast16_t) synthK3 * x2) >> 7);
#ifdef FAST_8BIT_MODE
    u1 = u2 - (((int_fast16_t) synthK2 * x1) >> 7);
    u0 = u1 - (((int_fast16_t) synthK1 * x0) >> 7);
#else
...
// Lattice filter reverse path -> compute next x* values
    x9 = x8 + (((int_fast16_t) synthK9 * u8) >> 7);
    x8 = x7 + (((int_fast16_t) synthK8 * u7) >> 7);
    x7 = x6 + (((int_fast16_t) synthK7 * u6) >> 7);
    x6 = x5 + (((int_fast16_t) synthK6 * u5) >> 7);
    x5 = x4 + (((int_fast16_t) synthK5 * u4) >> 7);
    x4 = x3 + (((int_fast16_t) synthK4 * u3) >> 7);
    x3 = x2 + (((int_fast16_t) synthK3 * u2) >> 7);
#ifdef FAST_8BIT_MODE
    x2 = x1 + (((int_fast16_t) synthK2 * u1) >> 7);
    x1 = x0 + (((int_fast16_t) synthK1 * u0) >> 7);
#else
...
static void setNextSynthesizerData() {
// 396 byte compiled
uint_fast8_t energy = sPointerToTalkieForISR->getBits(4);
